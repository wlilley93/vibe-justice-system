...
Before submission, the ruling is **anonymised**: repo names, file paths, variable names, function names, class names, and any project-specific identifiers are replaced by generic placeholders; the legal question, the ratio, the tier, the law applied, and the outcome are preserved unchanged. Community rulings are **persuasive precedent** across all VJS jurisdictions (any other repo running VJS). The clerk reviews each PR for constitutional compliance and subject-matter jurisdiction before merging. Supreme Court rulings that enact statute additionally touch `CASE-LAW.md` (VPR 6) in the same publication.

**Publication is a mandatory, non-skippable step, not a manual afterthought.** A Supreme Court judgment is **not complete** until it has been published to the Community Record; an unpublished SC ruling is a known breach of this rule (s. 5) the moment the judgment lands. The publication is performed by the deterministic step **`court/scripts/publish-ruling.sh <citation-slug> [--enacts]`**, which fails closed unless the anonymised ruling (and any enacted CASE-LAW article) is present, then branches, commits, pushes, and opens the PR via `gh`. The runnable courts (`court/workflows/*`) must invoke this step as their final phase, or the one writer must run it immediately on the court's return; either way the ruling is published the same turn it is decided. (Historic gap, closed 2026-06-05: the runnable Supreme Court workflow had a "Community PR" phase in design but, when re-authored into the executable inline format, the phase was dropped and rulings were left unpublished. This rule and the script make the omission impossible to repeat silently.)
