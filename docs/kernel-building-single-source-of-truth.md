...
brownfield specs (`kernel-specs/agent-libos.md`, `kernel-specs/opbox-kernel.md`,
`kernel-specs/vjs-kernel.md`), captured 2026-06-25. It defines a **set of global invariants,
each bound to a test**, and measures the VJS and Opbox kernels against it with a remediation
...
Opbox kernel has an authenticated bearer/grant model with scope fences and RLS at the verb
chokepoint. Agent libOS is the **reference exemplar** for a few specific properties of that
...
|---|---|---|
| Capability primitive (all three have one) | **Agent libOS** (cleanest of three) | the full capability record, deny-dominance, one-shot, attenuating delegation, revocation-on-next-check, typed resources, names-aren't-capabilities - the properties we harvested into the unified primitive |
| Invariant->test binding | **Agent libOS** | the meta-gate that fails the build if any claim lacks a deterministic test (the discipline that makes everything else real) |
| Irreversible-effect + human authority | **Agent libOS** | effect reversibility classification + blocking, one-shot, decided-once approval queue |
| Runtime chokepoint + identity | **Opbox** | one in-process dispatch, identity authenticated-by-construction, door-stamped immutable source |
| Storage-layer isolation + audit | **Opbox** | RLS (FORCE, non-superuser, null-GUC->0), encrypt-before-write with AAD, hash-chained audit in the write tx |
| Compile-time fail-closed + drift-proof attrs | **Opbox** | dev/no-engine paths excised from release; derived attributes + CI gates on undeclared egress/stub/schema |
...
reversibility/approval from Agent libOS (the reference exemplar, since all three already carry a
capability primitive at differing maturity); the runtime chokepoint + identity + RLS + audit +
compile-time fail-closed from Opbox; the entrenched floor + constitutive split + instrument-naming +
...
```
  ...
  ACTION/ENFORCEMENT plane (Opbox) the sole chokepoint: identity, capability check, the gate
        |  the only path to
  SUBSTRATE plane         (libOS)  the primitive boundary that actually touches the resource,
...
in brackets [L]=Agent libOS, [O]=Opbox, [V]=VJS.
...
and an Opbox "bearer/grant" both become *profiles* of this one record. The capability lifecycle governs
...
absent. (Agent libOS shown as the reference column.)

| # | Invariant | libOS | VJS | Opbox |
|---|---|---|---|---|
...
runtime chokepoint, audit hardening, and reversibility/approval. Opbox owns G1+G3+G5+G7 and is empty on the
...
1. **[BOTH] Stand up the invariant->test binding meta-gate (K-29, K-30) FIRST.** Port Agent libOS's
   ...
   map (43/281); Opbox proves citation not property - both upgrade to the binding gate.
2. **[Opbox] Make the write-path structural (K-1).** Attach the BD-5 `via_verb` trigger to kernel-owned
...
3. **[Opbox] Give the governance plane a real envelope or remove it (K-15, K-16, K-17).** The GOV permit gate
   ...
   as-enforcement. This is where Opbox adopts VJS's G4.
4. **[VJS] Resolve the runtime chokepoint (K-1, K-2, K-3).** VJS is commit-time by nature; cede runtime
   enforcement to the Opbox action plane (VJS as the consulted policy engine) rather than pretending the
...
5. **[BOTH] Converge on the unified capability primitive (K-4..K-11)** (Part III), generic over an opaque
   resource vocabulary, harvesting the cleanest properties from Agent libOS's exemplar. All three kernels
   already hold a capability primitive; this unifies them so VJS permits and Opbox bearers become profiles of
...
8. **[Opbox] Digest-pin the enforcement surface (K-25);** **[VJS] compile-time fail-closed + stub ratchet
   (K-26, K-30).** Cross-adopt each other's integrity mechanisms.

## Part VI - The keystone discipline (why K-29 comes first)

The three recent VJS bugs (permit-closed bypass, in_force mislabel, prose-boundary leak) and Opbox's inert-
governance and unattached-writeguard gaps share one root: **safety claims not bound to a failing-by-default
test.** Agent libOS's `check_test_invariants.py` is the cheapest, highest-leverage artifact in any of the
