...
1. **One invariant set, machine-checked.** Every governance rule that matters is expressed
   ...
   network) at the commit gate and in CI. Subscribing repos (Opbox, Agent libOS) overlay onto
...
VJS-side is the lane; the Opbox kernel is another agent's. Canon prose stays generic (no
subscriber repo codes / vertical names). Out-of-lane remainders are handed off, not actioned
here: the stale Opbox vendored mirror (canon is ahead), the canon-note relay to the Opbox
