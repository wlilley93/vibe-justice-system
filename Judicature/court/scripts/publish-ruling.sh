...
#   --enacts         the ruling enacts new CASE-LAW article(s) already appended to CASE-LAW.md (VPR 6)
#
...
#   - if --enacts: CASE-LAW.md already carries the new article(s).
...
FILES=("$RULING")
[ "$ENACTS" = "--enacts" ] && FILES+=("CASE-LAW.md")
git add "${FILES[@]}"
...
gh pr create \
  ...
  --body "Automated VPR 8 publication of Supreme Court ruling ${SLUG}, anonymised per VPR 8.${ENACTS:+ Enacts new CASE-LAW article(s) per VPR 6; clerk to review constitutional compliance before merge.}" \
