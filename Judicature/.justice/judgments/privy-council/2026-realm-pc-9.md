...
- Governance (VPR/SPEC-LAW/LEXBY caselaw): strict severance. The treatise may cite binding governance (as a textbook cites the statute constraining the practice) but may not restate, paraphrase, enact, or enter the LEXBY namespace.
- Skills catalogue: the treatise is the canopy above the leaves - it weaves the atomic skills + harvested learnings (L1-L8) into coherent chapter doctrine; it references skills, never duplicates a skill's content as a second source.
- Per-issue product records (RIDGELINE-ISSUE-NN): the records are the raw worked examples; the treatise is doctrine distilled from them, one-directional and provenance-filtered - only the generic, anonymised proposition/method, never the specific WHAT.
