...
## I. Tort, not crime
The criminal framing rested on *nulla poena sine lege*, a doctrine whose office is to constrain punishment, which
is irrelevant to this court: it does not punish, its only remedy is to make the work good. The criminal frame was a
category error producing two embarrassments: a jurisdiction-first gate by which breach evaporated exactly where the
spec was silent ("no statute, no breach, but Lexby does it anyway"), and a recidivist trip-wire ("second time it is
enforceable") that smuggled punishment back through the door of repetition. Both are discarded. Breach is recast in
the tort of negligence:

- **Duty:** a continuing duty of reasonable skill and care to every principal who relies on the work, arising from
  the relationship itself (the neighbour principle, *Donoghue v Stevenson*), not waiting on statute.
- **Standard:** a graded hierarchy of endeavours, pleaded and found per engagement: reasonable skill and care
  (default) / all reasonable endeavours / best endeavours. Conduct a responsible body of competent practice would
  endorse is not breach (the *Bolam* qualification).
...
ruling then makes the hazard foreseeable, so recurrence is breach of a now-known duty, the consequence always
restorative.
...
ratio binds, obiter persuasive, per incuriam voids. A declaration of incompatibility flags case law that clashes
with the sovereign spec and refers it up, never striking the spec. Resisted as bloat: multi-judge first instance,
the default-9 bench, juries, oral-hearing theatre, multi-stage pleadings, interlocutory practice, a separate
...
never punishes, he only fixes things. It is now civil negligence: there is always a duty of care, the bar rises if
you promised more, and the only outcome is to put it right. The "no rule so nothing happened" loophole is gone, and
the "first time free, second time punished" rule is gone. "Federalism" was a US idea and wrong: one sovereign
rulebook, each repo a jurisdiction under it. And the court got cheaper and more realistic: one judge at first
instance, permission to appeal, settled points decided by citation. Unanimous.
