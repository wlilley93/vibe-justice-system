...
# hard, deterministic gate for the RECORD: it fails closed, with no model in the loop, on three
# ways a jurisdiction silently corrupts itself:
#   - a newly added central judgment file without explicit court-workflow or authorised-registrar
#     provenance metadata,
...
# It runs `cdd check`; if the record is sound it is silent and exits 0, otherwise it
...
if ! run_cdd check; then
  echo "" >&2
  echo "VJS hard gate: the deterministic CLI audit failed (see above). The commit is blocked" >&2
  echo "because the legal record would be left unauthorised-looking or inconsistent." >&2
  echo "Fix the judgment provenance/citator/bench-name findings, or, for a deliberate" >&2
