...
| Codex-style sessions | root `AGENTS.md`; session/developer instructions; `cdd` deterministic checks | manual / instruction-level in this repository; no automatic Codex hook runner found | supported where subagents are available | use subagents for separable work, run `cdd` checks, record that automatic trigger is unavailable | presence of root `AGENTS.md`; command output from `cdd` checks |
