...
| Git commit workflow | `.git/hooks/pre-commit` chained or symlinked to `.vjs/hooks/vjs-pre-commit.sh` | automatic when installed in the git repository | not applicable | run `bash .vjs/hooks/vjs-pre-commit.sh` or `cdd check` manually | `cdd init`; `.git/hooks/pre-commit`; `cdd check` |
| Git public-push workflow | `.git/hooks/pre-push` chained or symlinked to `.vjs/hooks/vjs-pre-push.sh` | automatic when installed in the git repository | not applicable | do not push publicly until local CI passes and release warrant exists; retrieve the warrant with `cdd release-warrant`; run pre-push gate manually if needed | `cdd init`; `.git/hooks/pre-push`; `cdd local-ci`; `cdd release-warrant`; [2026] REALM-SI 7 warrant |
...
`vjs-pre-push.sh` invokes `cdd local-ci` for the canonical public remote before it evaluates the
release warrant.
