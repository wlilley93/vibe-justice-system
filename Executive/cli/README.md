...
| `cdd local-ci [--json]` | Run local, no-hosted-CI VJS verification: syntax checks, JSON parse checks, public-law index consistency, Gazette privacy scan, hook reminder smoke, CLI tests, deterministic aggregate checks, law/graph smoke checks, and whitespace checks. Public-release hooks call this before warrant validation. |
...
| `cdd law search "<query>" [--kind case\|bill\|si\|submission\|all] [--limit N] [--json]` | Token-efficient search over `site/search-index.json`. Results are pointer records only and are retrieval aids, not legal force. Default search ranks legal authority ahead of non-law submissions. |
...
| `cdd gazette privacy [--json]` | Deterministically scan generated public Gazette artifacts for private operational data before release. Included in `cdd local-ci`. |
| `cdd gazette live-check [--base-url URL] [--json]` | Fetch deployed GitHub Pages Gazette artifacts and compare them with the local generated `site/` artifacts after publication. |
