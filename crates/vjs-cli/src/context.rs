...
// (A doc comment describing `vjs gazette` sat here, attached to `load_lawpack`, which it
// did not describe. The command it documents lives in gazette.rs. Dropped rather than
// carried onto a `use`.)
// Lawpack resolution is the ENGINE's, re-exported so the call sites in this crate read
