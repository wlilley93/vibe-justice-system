...
fn mcp_allocate_segments_cc_citations_by_repo() {
    ...
    let opbox = srv
        .handle_request(
            r#"{"jsonrpc":"2.0","id":1,"method":"vjs.allocate","params":{"series":"CC","repo":"opbox","year":2026}}"#,
        )
        .expect("allocate cc");
    assert!(
        opbox.contains("VJS-CC-OPBOX"),
        "a Cc allocation must carry the repo segment: {opbox}"
    );
...
fn mcp_record_refers_a_subscribers_above_county_order_up() {
    ...
    let supreme = r#"{"jsonrpc":"2.0","id":1,"method":"vjs.record","params":{"id":"TK-SUP-1","court":"supreme_court","jurisdiction":"opbox","repo_code":"OPBOX","status":"binding","issue":"tk/apex","holding":"x","directives":[],"forbidden":null,"exceptions":null,"supersedes":[],"source_opinion":null,"runtime_summary":"t","created_at":"2026-06-26"}}"#;
