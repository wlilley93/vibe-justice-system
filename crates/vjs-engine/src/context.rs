//! THE kernel context, for every door ([2026] VJS-CC-VJS 16 C4).
//!
//! There were TWO. The CLI built its graph here (via `vjs-cli/src/context.rs`) and overlaid
//! this repository's own filed orders; the MCP server built its own in `vjs_mcp::build_context`
//! and did not. Measured 2026-08-01 on a clean fixture with one order in `.vjs/orders` and
//! nothing vendored: `vjs lookup --issue scratch-fixture` returned the order FIRST and then
//! the constitutional stack, while MCP `vjs.lookup` on the same issue in the same repository
//! returned 151 authorities, every one of them canon, and not the order.
//!
//! Which means the only reason the MCP could ever read back what it recorded was that its
//! write DISPLACED the canon: the hardcoded write target was load-bearing for a coherence the
//! server otherwise lacked. Moving the write (C1) without moving the reader would have left
//! a verb that records into a register the same server cannot read. Two context builders are
...
/// jurisdiction's own filed orders.
