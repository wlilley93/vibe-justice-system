...
import { configFile, R } from "../paths.js";
...
export function loadConfig(): Config {
  ...
  if (process.env.VJS_PROVIDER) cfg.provider = process.env.VJS_PROVIDER;
  if (process.env.VJS_MOCK_LLM === "1") cfg.provider = "mock";
  return cfg;
}

function logCall(caseId: string, promptName: string, provider: string, ms: number, chars: number) {
  try {
    // One log for the jurisdiction. A court has matters, not cases, and a matter may
    // outlive any one of them, so the log is keyed by matter in the line rather than
    // by directory.
    fs.mkdirSync(R(".court"), { recursive: true });
    const f = R(".court", "bench.log.jsonl");
    fs.appendFileSync(f, JSON.stringify({ matter: caseId, promptName, provider, ms, chars, at: new Date().toISOString() }) + "\n");
 ...
 * with VJS_FIXTURES pointed at it.
 *
...
function recordCompletion(caseId: string, promptName: string, provider: string, text: string) {
  try {
    const dir = R(".court", "opinions", caseId || "adhoc");
...
export async function getProvider(cfg: Config): Promise<LLMProvider> {
  ...
  throw Object.assign(new Error("no LLM provider available (no claude CLI, no ANTHROPIC_API_KEY); use VJS_MOCK_LLM=1"), { code: 5 });
}
