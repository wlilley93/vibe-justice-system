...
function describe(o: Owner | null): string {
  if (!o) return "another vjs command is running in this repo";
  const age = Math.round((Date.now() - Date.parse(o.at)) / 1000);
  return `another vjs command is running in this repo (pid ${o.pid}, \`${o.cmd}\`, started ${age}s ago)`;
}
