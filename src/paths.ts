// The only module that knows where the jurisdiction is.
//
// Before this existed, every path in src/ was bare-relative ("cases/", "lean/", ".justice/",
// "vjs.config.json", …), so `process.cwd()` WAS the repo root by assumption and any host
...
// user-supplied relative arguments — `vjs rule --from ../questions/q.json` currently resolves
...
function discover(from: string): string | null {
  ...
  for (;;) {
    if (fs.existsSync(path.join(dir, "vjs.config.json"))) return dir;
...
/** explicit (--root) > VJS_ROOT > walk up from cwd > cwd itself. */
...
export const justice   = (...p: string[]) => R(".justice", ...p);
export const recordDir = () => R("record");
export const lawDir    = () => R("law");
...
export const fixturesDir = () =>
  process.env.VJS_FIXTURES ? path.resolve(process.env.VJS_FIXTURES) : R("fixtures");
...
export const configFile = () =>
  process.env.VJS_CONFIG ? path.resolve(process.env.VJS_CONFIG) : R("vjs.config.json");
